...
function doc_setup() {
	...
	companiesView = Backbone.View.extend({
		...
		renderModifyPage : function(id) {
			...
			var company = Companies.getModelById(id);
			var companyJSON = company.toJSON();
			var html = ich.modify_company_page_TMP(_.extend({
				company : companyJSON,
				company_id : id
			}, breadCrumb(id)));
			$("#main").html(html);
			$('#form').find('input').attr("disabled", true);
			$("#dialog-hook").html(ich.companyInputDialog_TMP({
			...
			$("#btnModifyRewards").click(function() {
				function saveRewardsProgram() {
					return function(mobqreditsconversion, qriketconversion) {
						...
						rewardsModel.save({
							use_mobqredits : rewardsJson.use_mobqredits,
				...
				fetch_company_rewards(companyJSON._id)(function(err, rewards) {
					console.log(rewards);
					rewardsModel = rewards;
					var rewardsJson = rewardsModel.toJSON();

					var html = ich.companyModifyRewardsDialog_TMP({
